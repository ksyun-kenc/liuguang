	...
	for (i = 0; i < formatList->numFormats; i++)
	{
		...
		if (format->formatName)
		{
			int size = MultiByteToWideChar(CP_UTF8, 0, format->formatName,
										   (int)strlen(format->formatName), NULL, 0);
			mapping->name = calloc(size + 1, sizeof(WCHAR));

			if (mapping->name)
			{
				MultiByteToWideChar(CP_UTF8, 0, format->formatName, (int)strlen(format->formatName),
									mapping->name, size);
	...
	if (requestedFormatId == RegisterClipboardFormat(CFSTR_FILEDESCRIPTORW))
	{
		...
		else
		{
				 ...
				 p += len + 1, clipboard->nFiles++)
			{
				...
				MultiByteToWideChar(CP_ACP, MB_COMPOSITE, p, (int)len, wFileName, cchWideChar);
...
BOOL wf_cliprdr_init(wfClipboard *clipboard, CliprdrClientContext *cliprdr)
{
	...
	if (!(clipboard->data_obj_mutex = CreateMutex(NULL, FALSE, _T("data_obj_mutex"))))
		goto error;
